...
import { 
  ...
  AlertTriangle, CheckCircle, Ghost, List
} from 'lucide-react';
import { supabase, StoryEvent, EventStatus, StoryLegend } from '@/lib/supabase';
import { useAuth } from '@/hooks/useAuth';

export default function Admin() {
  const { user, loading: authLoading } = useAuth();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<'events' | 'legend'>('events');
  ...
  async function fetchData() {
    setLoading(true);
    await Promise.all([fetchEvents(), fetchLegend()]);
    setLoading(false);
  }
    ...
    <div className="py-12 px-4 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6 font-display">
        ...
        <div className="flex bg-white/5 p-1 border border-white/10">
          ...
          >
            <Ghost className="w-4 h-4" /> Truyền Thuyết
          </button>
        </div>
      </div>
